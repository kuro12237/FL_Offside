...
struct ShaderTag {
  ...
  NLOHMANN_DEFINE_TYPE_INTRUSIVE(ShaderTag, PiplineName, PS, VS, DS, HS, GS, CS,
                                 RAYGEN, MISS, CLOSEST_HIT, ANY_HIT,
                                 INTERSECTION)
};
...
public:
  ...
  IDxcBlob *GetBlob(CLEYERA::Graphics::RasterPipline_Mode3d pipMode,
                    Shader::ShaderMode shaderMode) {
    return compornemts_[pipMode]->GetShader(shaderMode);
  };
  IDxcBlob *GetBlob(CLEYERA::Graphics::RasterPipline_Mode2d pipMode,
                    Shader::ShaderMode shaderMode) {
    return compornemts2d_[pipMode]->GetShader(shaderMode);
  };
  IDxcBlob *GetBlob(CLEYERA::Graphics::PostEffect_Mode pipMode,
                    Shader::ShaderMode shaderMode) {
    return compornemtsPostEffect_[pipMode]->GetShader(shaderMode);
  };
  ...
  ShaderTag GetFIleName(RasterPipline_Mode2d mode) {
    return fileNames2d_[mode];
  }
  ShaderTag GetFIleName(PostEffect_Mode mode) {
    return fileNamesPostEffect_[mode];
  }
...
private:
  ...
  std::map<PostEffect_Mode, ShaderTag> fileNamesPostEffect_;

  std::map<RasterPipline_Mode3d, std::shared_ptr<ShaderCompornent>>
      compornemts_;
  std::map<RasterPipline_Mode2d, std::shared_ptr<ShaderCompornent>>
      compornemts2d_;
  std::map<PostEffect_Mode, std::shared_ptr<ShaderCompornent>>
      compornemtsPostEffect_;
};
